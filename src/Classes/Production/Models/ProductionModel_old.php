...
class ProductionModel
{
    ...
    public function __construct(Connection $dbConnection, LoggerInterface $log, Utilities $util)
    {
        ...
        /** @var \Database\Connection $dbConnection */
    ...
    public function insertProdLog($prodData, $materialData, $tempData)
    {
        try {
            ...
            */
            ...
            $this->updateProductionRun($prodRunID);
            ...
            if ($prodData['runStatus'] === 'end') {
                ...
                $this->markProductionRunCompleted($prodRunID);
            }
    ...
    */
     ...
     * @return void
     */
    private function updateProductionRun($prodRunID)
    {
    ...
    private function markProductionRunCompleted($prodRunID)
    {
        $this->log->info("markProductionRunCompleted called for prodRunID: {$prodRunID}");

        // Get the last production date so we can set the official end date 
        $sqlLastDate = "SELECT lastProdDate FROM prodrunlog WHERE logID = :prodRunID";
    ...
    public function getInvQty($productID)
    {
        ...
        $this->log->info("Current inventory for {$productID} using getInvQty() function is {$qty}.");
