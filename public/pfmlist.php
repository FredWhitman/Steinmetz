            ...
            <div class="mt-5">
                <div class="row">
                    <div class="col d-flex justify-content-center">
                        <div class="card" >
                            <div class="card-header"><h2 class="text-center">PFM List</h2></div>
                            <div class="card-body">
                                <?php if(isset($_GET['updated'])){}?>
                                <div class="table-responsive" style="height:550px; overflow-y: auto">
                                    <table class="table table-striped table-bordered table-hover table-sm">
                                    <thead><tr><th class="text-center">PFM ID</th><th class="text-center">Part Number</th><th class="text-center">Part Name</th><th class="text-center">Product ID</th><th class="text-center">Min Qty</th><th class="text-center">Amsted PFM</th><th class="text-center"></th></tr></thead>
                                        <?php try{$query = "SELECT * FROM pfm";$stmt = $objPFM->runQuery($query);$stmt->execute();}catch(PDOException $e){echo 'ERROR! :';echo $e->getMessage();}?>
                                        <tbody>
                                            <?php if($stmt->rowcount()>0){while($rowPFM = $stmt->fetch(PDO::FETCH_ASSOC)){?>
                                            <tr>
