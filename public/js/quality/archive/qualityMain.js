  ...
  form.addEventListener("submit", async (e) => {
    ...
    const payload = {
      ...
      matTransData: {
        ...
        deliveryDate: data.get("mr_matReceivedDate"),
        inventoryLogID: "0",
        oldStockCount: "0",
...
function addPFMReceived() {
  const form = document.getElementById("add-pfmreceived-form");
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    if (!form.checkValidity()) {
      form.classList.add("was-validated");
    ...
    const payload = {
      action: "pfmReceived",
      pfmTransData: {
        inventoryID: data.get("pr_partNumber"),
        inventoryType: "pfm",
        deliveryDate: data.get("pr_pfmReceivedDate"),
        inventoryLogID: "0",
        oldStockCount: "0",
        transAmount: data.get("pr_qtyReceived"),
        transType: "received",
        transComment: data.get("pr_Comments"),
      },
    };
    console.log("PFM Received Payload:", JSON.stringify(payload, null, 2));

    try {
      const result = await postPfmReceived(payload);

      if (result) {
        const alertData = result;
        document.getElementById("showAlert").innerHTML = alertData.html;
        bootstrap.Modal.getInstance(
          document.getElementById("addPfmReceivedModal")
        ).hide();
      }
      console.log("postReceivedShipments result: ", result);
    } catch (error) {
      console.error("Failed to submit received shipments:", error);
      document.getElementById("showAlert").innerHtml;
    }
  });
}
...
async function onPFMReceivedModalShow() {
  showLoader();
  clearAlert();
  try {
    const pfms = await fetchPFMList();

    if (!Array.isArray(pfms)) {
      showAlertMessage(
        "⚠️ PFM lists failed to load properly.",
        "showAlert",
        "danger"
      );
      console.error("pfms", pfms);
      return;
    }
    console.log("pfms", pfms);
    const sel = document.getElementById("pr_partNumber");
    populatePFMSelect(sel, pfms);
  } catch (error) {
    console.error(error);
    showAlertMessage("Unable to load PFM list!", "showAlert", "danger");
  } finally {
    hideLoader();
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const addPfmReceivedModal = document.getElementById("addPfmReceivedModal");
  addPfmReceivedModal.addEventListener("show.bs.modal", onPFMReceivedModalShow);
