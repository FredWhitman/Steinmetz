  ...
  if (!ovenLogs.length) {
    ...
    ovenLogs.forEach((row) => {
      ...
      if (row.inOvenTime && row.outOvenTime) {
        ...
        console.log("inTime in minutes: ", inTime);
        const outTime = new Date(`1970-01-02T${row.outOvenTime}`);
        console.log("outTime in minutes: ", outTime);
      ...
      if (timeDiffMinutes !== null) {
        if (timeDiffMinutes < 960 || timeDiffMinutes > 1440) {
...
export function renderReceivedShipmentsTable(receivedShipments) {
  const container = document.getElementById("receivedShipments");
  if (!receivedShipments || receivedShipments.length === 0) {
    container.innerHTML =
      "<tr><td colspan='11'>No received shipment data available.</td></tr>";
    return;
  } else {
    document.getElementById("receivedshipments").innerHTML =
...
export function populatePFMSelect(selectEl, pfms) {
  if (!selectEl) {
    console.warn("🚨 Select element not found!");
    return;
  }
  console.log("qualityUiManager.js->populatePFMSelect() called");

  populateSelect(selectEl, pfms, {
    valueKey: "partNumber",
    labelKey: "partName",
  });
}
