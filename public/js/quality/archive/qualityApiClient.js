...
export async function fetchPFMList() {
  const res = await fetch(`${BASE_URL}?action=getPfms`);
  return handleResponse(res);
}
...
export async function postPfmReceived(payload) {
  console.log("postPfmReceived called!");
  const res = await fetch(BASE_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
...
export async function fetchReceivedShipments() {
  const res = await fetch(`${BASE_URL}?action=getReceivedShipments`);
  return handleResponse(res);
}
